...
2. To understand why this needs to be nonisolated [here](https://www.avanderlee.com/swift/actors/) is an amazing post to learn everything about actors. In general remote config values can be anything, Boolean, Integer, String, custom data types, etc. The access method therefore needs to be generic as well. But why is the generic value constraint to be LosslessStringConvertible? Firebase SDK only offers to unwrap the remote config value to a string-, bool-, data-, json- and number value. More or less all values can be represented as String so it made sense to first get the untyped value as a String and later type cast it to the desired type. To make sure this type cast can work Value needs to be initialisable from a string.
